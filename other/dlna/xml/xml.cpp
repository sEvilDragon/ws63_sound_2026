                                      ...
                                      const char *udn)
{
    ...
    int ret = snprintf(out, out_size,
                       ...
                       udn);
    if (ret < 0 || out[0] == '\0') {
        osal_printk("dlna xml build failed: desc ret=%d first=%d size=%u\n", ret, static_cast<int>(out[0]),
                    static_cast<unsigned int>(out_size));
    } else {
        osal_printk("dlna xml build ok: desc len=%d first=%c\n", ret, out[0]);
    }
    return (ret < 0) ? ERRCODE_FAIL : ERRCODE_SUCC;
}
