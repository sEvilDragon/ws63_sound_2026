...
```
物理布局:
   功能键:  [1=A]  [2=B]  [3=C]                   ← 3 个独立按键
   滑条:    [4] [5] [6] [7] [8] [9] [10] [11]    ← 8 段位置滑条 (实际 pin 见 PAD_SLIDER)
```
...
```cpp
PAD_SLIDER[8] = {9, 10, 11, 12, 16, 15, 14, 13};
//  index        0   1   2   3   4   5   6   7   ← 逻辑位置 pos
```

> 功能键对应低位 pin: `PAD_FUNC_A=1, B=2, C=3`.
> 滑条使用高位 pin 9..16, 物理从左到右为 9→10→11→12→16→15→14→13.
> 如果发现滑动方向反了, 把整个 `PAD_SLIDER` 数组 reverse 即可.
> 如果某位置不响应, 对照 `raw` 日志确认对应 bit 后自行调整 pin 号.

### 4.2 位置计算：加权质心 (weighted centroid)

**不用 rightmost-peak**：右手峰在相邻 pad 同时激活时, 若 PAD_SLIDER 索引与该 pad 物理位置不对齐, 会出现位置回跳（明明向右滑, pos 却从 6 跳回 4）。

**当前算法：对 PAD_SLIDER 中所有被按下的 pad, 取其逻辑索引 (0..7) 的加权平均, 结果放大 ×100 得到 0..700 的平滑位置。**

```cpp
sum_index = Σ(i)  for each active pad at index i
count     = number of active slider pads
pos       = (sum_index × 100) / count   // range 0..700
```

特性：

- 相邻 pad 同时激活时得到插值 (如 index 2+3 → pos=250 → 逻辑 2.50)
- 单向 swipe 产生单调变化的 pos 序列 (不会回跳)
- 100 倍缩放提供亚位精度
...
```
PAD_FUNC_A = 1    // 低位独立按键 A
PAD_FUNC_B = 2    // 低位独立按键 B
PAD_FUNC_C = 3    // 低位独立按键 C
```
...
| 参数 | 值 | 含义 |
|------|----|------|
| `FUNC_DEBOUNCE_TICKS` | 2 | 连续 2 帧一致才接受（≈ 66ms） |
| `FUNC_HOLD_TICKS`     | 20 | 接受后 ~66ms 内忽略反向抖动 |
