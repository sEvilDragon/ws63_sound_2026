...
private:
    ...
    ;                                                   // 读写权限，写无响应权限
    static constexpr uint16_t property_value_len = 0;   // 属性初始值长度（与C原始代码一致）
    static constexpr uint8_t *property_value = nullptr; // 属性初始值

    // 定义音频服务的广播
    static constexpr uint8_t audio_announce_handle = 01; // 广播句柄，用于确认是哪个服务正在广播
    static constexpr uint16_t audio_announce_mode =
        SLE_ANNOUNCE_MODE_CONNECTABLE_SCANABLE;                                      // 广播数据包类型，不定向广播
    ...
    static constexpr uint16_t audio_announce_channel_map = 0x7;                      // 广播频道映射，所有频道
    static constexpr uint16_t audio_announce_interval_min = 0xC8;                    // 广播间隔，单位为毫秒
    static constexpr uint16_t audio_announce_interval_max = 0xC8;                    // 广播间隔，单位为毫秒
    static constexpr uint16_t audio_announce_tx_power = 20;
    // 广播连接后自动更新间隔
    static constexpr uint16_t high_speed_interva_min = 0x14; // 传输间隔最小值（SLE协议最小合法值750?s）
    static constexpr uint16_t high_speed_interva_max = 0x14; // 传输间隔最大值（SLE协议最小合法值750?s）
    ...
    static constexpr std::array<uint8_t, 7> advertising_data = {
        0x01,
        0x01,
        0x01, // 广播级别，后面有一个字节，常规可发现
        0x05,
        0x02, // 完整16位服务UUID列表，后面有2个字节
        uuid_service_audio_1,
        uuid_service_audio_2, // 服务的uuid
    };
    ...
    static constexpr uint8_t phy_pilot_density = SLE_PHY_PILOT_DENSITY_16_TO_1; // 导频间隔16
