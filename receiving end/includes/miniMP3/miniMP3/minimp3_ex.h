...
#define MP3D_SEEK_TO_SAMPLE                                                                                     \
    1 /* mp3dec_ex_seek precisely seeks to sample using index (created during duration calculation scan or when \
         mp3dec_ex_seek called) */
#define MP3D_DO_NOT_SCAN                                                                                        \
    2 /* do not scan whole stream for duration if vbrtag not found, mp3dec_ex_t::samples will be filled only if \
         mp3dec_ex_t::vbr_tag_found == 1 */
...
#define MP3D_E_DECODE \
    -5 /* decode error which can't be safely skipped, such as sample rate, layer and channels change */
...
typedef int (*MP3D_ITERATE_CB)(void *user_data,
                               const uint8_t *frame,
                               int frame_size,
                               int free_format_bytes,
                               size_t buf_size,
...
int mp3dec_load_w(mp3dec_t *dec,
...
// 允许外部通过宏自定义 IO 缓冲区分配策略（如使用静态预分配缓冲）。
#ifndef MINIMP3_ALLOC_IO
#define MINIMP3_ALLOC_IO(size) malloc(size)
#endif
#ifndef MINIMP3_FREE_IO
#define MINIMP3_FREE_IO(ptr) free((void *)(ptr))
#endif
...
#define MINIMP3_ID3_DETECT_SIZE 10
    ...
    if (buf_size >= MINIMP3_ID3_DETECT_SIZE && !memcmp(buf, "ID3", 3) &&
        ...
        size_t id3v2size =
            (((buf[6] & 0x7f) << 21) | ((buf[7] & 0x7f) << 14) | ((buf[8] & 0x7f) << 7) | (buf[9] & 0x7f)) + 10;
        if ((buf[5] & 16))
            id3v2size += 10; /* footer */
        return id3v2size;
    }
                   ...
                   void *user_data)
{
    ...
    do {
        ...
        if (io) {
            if (!eof &&
                filled - consumed < MINIMP3_BUF_SIZE) { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
    ...
    do {
        ...
        if (io) {
            ...
            samples =
                mp3dec_decode_frame(dec, buf + consumed, filled - consumed, info->buffer + info->samples, &frame_info);
            consumed += frame_info.frame_bytes;
        } else {
            samples = mp3dec_decode_frame(dec, buf, MINIMP3_MIN(buf_size, (size_t)INT_MAX),
                                          info->buffer + info->samples, &frame_info);
...
static int mp3dec_load_index(void *user_data,
                             const uint8_t *frame,
                             int frame_size,
                             int free_format_bytes,
                             size_t buf_size,
                             ...
                             mp3dec_frame_info_t *info)
{
    ...
    if (!dec->index.frames && !dec->start_offset) { /* detect VBR tag and try to avoid full scan */
        uint32_t frames;
        int delay, padding;
        dec->info = *info;
    ...
    if (dec->index.num_frames + 1 > dec->index.capacity) {
        ...
        mp3dec_frame_t *alloc_buf =
            (mp3dec_frame_t *)realloc((void *)dec->index.frames, sizeof(mp3dec_frame_t) * dec->index.capacity);
        if (!alloc_buf)
            return MP3D_E_MEMORY;
    ...
    if (!dec->buffer_samples && dec->index.num_frames < 256) { /* for some cutted mp3 frames, bit-reservoir not filled
                                                                  and decoding can't be started from first frames */
        /* try to decode up to 255 first frames till samples starts to decode */
        dec->buffer_samples =
            mp3dec_decode_frame(&dec->mp3d, frame, MINIMP3_MIN(buf_size, (size_t)INT_MAX), dec->buffer, info);
...
static size_t mp3dec_idx_binary_search(mp3dec_index_t *idx, uint64_t position)
{
    ...
    while (start <= end) {
        ...
        if (idx->frames[mid].sample >= position) { /* move left side. */
...
int mp3dec_ex_seek(mp3dec_ex_t *dec, uint64_t position)
{
    ...
    if (0 == position) { /* optimize seek to zero, no index needed */
    seek_zero:
        dec->offset = dec->start_offset;
        dec->to_skip = 0;
        goto do_exit;
    }
    if (!dec->indexes_built) { /* no index created yet (vbr tag used to calculate track length or MP3D_DO_NOT_SCAN open
                                  flag used) */
        dec->indexes_built = 1;
        dec->samples = 0;
        dec->buffer_samples = 0;
        if (dec->io) {
            ...
            int ret = mp3dec_iterate_buf(dec->file.buffer + dec->start_offset, dec->file.size - dec->start_offset,
                                         mp3dec_load_index, dec);
            if (ret && MP3D_E_USER != ret)
                return ret;
        }
    ...
    if (i) {
        ...
        if (3 == dec->info.layer) {
            while (i && to_fill_bytes) { /* make sure bit-reservoir is filled when we start decoding */
                bs_t bs[1];
                L3_gr_info_t gr_info[4];
    ...
    if (position > dec->index.frames[i].sample) {
        ...
        dec->to_skip = 0;
    while ((i + 1) < dec->index.num_frames && !dec->index.frames[i].sample &&
           !dec->index.frames[i + 1].sample) { /* skip not decodable first frames */
        const uint8_t *hdr;
        if (dec->io) {
            hdr = dec->file.buffer;
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf, mp3dec_frame_info_t *frame_info, size_t max_samples)
{
    ...
    *buf = NULL;
    ...
    while (dec->buffer_consumed == dec->buffer_samples) {
        ...
        if (dec->io) {
            if (!eof && (dec->input_filled - dec->input_consumed) <
                            MINIMP3_BUF_SIZE) { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
                memmove((uint8_t *)dec->file.buffer, (uint8_t *)dec->file.buffer + dec->input_consumed,
                        dec->input_filled - dec->input_consumed);
                dec->input_filled -= dec->input_consumed;
                dec->input_consumed = 0;
                size_t readed = dec->io->read((uint8_t *)dec->file.buffer + dec->input_filled,
                                              dec->file.size - dec->input_filled, dec->io->read_data);
            ...
            if (!(dec->input_filled - dec->input_consumed))
                return 0;
            dec->buffer_samples = mp3dec_decode_frame(&dec->mp3d, dec_buf, dec->input_filled - dec->input_consumed,
                                                      dec->buffer, frame_info);
            ...
            if (!buf_size)
                return 0;
            dec->buffer_samples = mp3dec_decode_frame(&dec->mp3d, dec_buf, MINIMP3_MIN(buf_size, (uint64_t)INT_MAX),
                                                      dec->buffer, frame_info);
        }
        dec->buffer_consumed = 0;
        ...
        } else if (dec->to_skip > 0) { /* In mp3 decoding not always can start decode from any frame because of bit
                                          reservoir, count skip samples for such frames */
    ...
    if (dec->detected_samples) { /* count decoded samples to properly cut padding */
...
int mp3dec_ex_open_cb(mp3dec_ex_t *dec, mp3dec_io_t *io, int flags)
{
    ...
    dec->file.buffer = (const uint8_t *)MINIMP3_ALLOC_IO(dec->file.size);
    if (!dec->file.buffer)
        return MP3D_E_MEMORY;
#endif
    dec->flags = flags;
...
retry_mmap2:
    ...
    if (MAP_FAILED == map_info->buffer || buffer != (void *)map_info->buffer)
        goto error;
retry_mmap3:
    buffer = mmap((void *)map_info->buffer + map_info->size, map_info->size, PROT_READ | PROT_WRITE,
...
static int mp3dec_load_mapinfo(mp3dec_t *dec,
...
#else
    if (dec->io && dec->file.buffer)
        MINIMP3_FREE_IO(dec->file.buffer);
#endif
    if (dec->index.frames)
        free(dec->index.frames);
    memset(dec, 0, sizeof(*dec));
